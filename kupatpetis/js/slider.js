jQuery(document).ready(function($){
   ...
   //setSelectedPost();   //set title, excerpt and link for first selected post
   //$('.selected-post').show();
   
   startTimer();
   
   $('.site-wrap', '#home-slider-outer').hover(
       function(){
           stopTimer = true;
            setTimeout(function(){
                if( stopTimer ) {
   ...
   *************** Custom Functions *****************/
   function startSlider() {
       ...
       $('ul#slider-list').animate({'left': shiftLeft}, 1000, setSelectedPost);
   }
   
   function imageSizing() {
       slideWidth = $('#home-slider').width();
   ...
   function setSelectedPost() {
       var postLink = $('a', 'li#slider-' + selectedSlide).attr('href');
       var postTitle = $('#slide-post-title-' + selectedSlide).text();
       var postExcerpt = $('#slide-post-excerpt-' + selectedSlide).text();
       $('.selected-post-title').text(postTitle);
       $('.selected-post-excerpt').text(postExcerpt);
       $('.more-link', '.selected-post').attr('href', postLink).show();
   }
