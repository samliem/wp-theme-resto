...
function jrl_resto_register_setting() {
    //register_setting( $option_group, $option_name, $sanitize_callback
    register_setting( 'jrl_resto_theme_options', 'jrl_resto_theme_options', 'resto_theme_options_validate' );
}

add_action( 'admin_init', 'jrl_resto_register_setting' );

function resto_theme_options_validate($options) {
    
    $old_options = (array) get_option('jrl_resto_theme_options');
    ...
    if( isset($options['resto']) ) {
        $options['resto'] = strip_tags($options['resto']);
    }
...
function jrl_options_init() {
    $jrl_theme_options = get_option('jrl_resto_theme_options');
    if( false === $jrl_theme_options ) {
        $jrl_theme_options = jrl_get_default_options();
        update_option( 'jrl_resto_theme_options', $jrl_theme_options );
    }
}
...
function jrl_menu_options() {
    ...
    add_theme_page( 'Theme Setting Option', 'Theme Options', 'edit_theme_options', 'resto-theme-setting', 'resto_admin_options_page' );
    if( !isset($_COOKIE['resto_tab']) ) {
        setcookie('resto_tab', 'general', time()+86400, '/wordpress/wp-admin/', COOKIE_DOMAIN);
    }
}
add_action( 'admin_menu', 'jrl_menu_options' );

function resto_admin_options_page() {
    ...
    <div class="wrap">
        ...
        <ul id="tabs">
            ...
            --><li <?php if( 'slider' == $_COOKIE['resto_tab'] ) echo 'class="tab-active"'; ?>>
                    <a href="#setting-slider" id="home-slider-tab">Home Slider</a>
            </li><!--
            --><li <?php if( 'social' == $_COOKIE['resto_tab'] ) echo 'class="tab-active"'; ?>>
        ...
        <?php $jrl_theme_options = get_option('jrl_resto_theme_options'); ?>
        <div id="tab-content">
